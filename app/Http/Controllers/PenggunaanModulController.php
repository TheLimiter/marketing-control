...
    }
    ...
    {
        // Ambil semua modul aktif untuk checklist
        $modul = Modul::query()
            ->when($r->boolean('only_active', true), fn($q) => $q->where('active', true))
            ->orderBy('nama')->get(['id','nama']);

        // Jika dipanggil dari halaman sekolah, bawa school id
        $schoolId = $r->integer('school') ?: null;

        // Render view form batch (lihat section C)
    ...
    public function storeBatch(Request $r)
    {
        // 1) Validasi request
        $data = $r->validate([
            'master_sekolah_id' => 'required|exists:master_sekolah,id',   // ganti ke klien_id jika pakai itu
        ...
        // 2) Eksekusi dalam transaksi
        $created = 0; $skipped = 0; $rows = [];

        DB::transaction(function () use ($schoolId, $modulIds, &$created, &$skipped, &$rows) {
            foreach ($modulIds as $mid) {
                // **Jika tabelmu pakai kolom 'klien_id', ganti baris di bawah ini**
                $pm = \App\Models\PenggunaanModul::firstOrCreate(
                    ['master_sekolah_id' => $schoolId, 'modul_id' => $mid],
                    [] // kolom default lain jika ada
                );
        ...
        // 3) Log RINGKAS (wajib) — satu entri untuk batch
        log_activity(
            'modul.use_batch',
            null,        // tidak ada satu subject spesifik
            [],          // before
        ...
        // 4) Log PER-MODUL (opsional, jika kamu mau jejak detail)
        // NB: Observer 'created' pada PenggunaanModul (kalau sudah kamu pasang) akan mencatat juga.
        // Jika observer SUDAH ada, kamu BISA lewati loop ini.
        /*
        foreach ($rows as $pm) {
            log_activity('modul.use_add', $pm, [], ['modul_id' => $pm->modul_id, 'id' => $pm->id], $schoolId, 'Modul ditambahkan');
        }
        */
